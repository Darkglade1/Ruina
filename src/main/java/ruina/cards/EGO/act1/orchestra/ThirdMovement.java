...
public class ThirdMovement extends AbstractEgoCard {
    public final static String ID = makeID(ThirdMovement.class.getSimpleName());
    public static final int DAMAGE = 14;
    public static final int UP_DAMAGE = 3;

    public ThirdMovement() {
        super(ID, 1, CardType.ATTACK, CardTarget.ALL_ENEMY);
        damage = baseDamage = DAMAGE;
        isMultiDamage = true;
        cardsToPreview = new FourthMovement();
        exhaust = true;
    }
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        atb(new AbstractGameAction() {
            @Override
            public void update() {
                AbstractCard c = getMasterDeckEquivalent(ThirdMovement.this);
                if (c != null) {
                    AbstractCard nm = new FourthMovement();
