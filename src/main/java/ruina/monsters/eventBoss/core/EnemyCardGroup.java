...
public class EnemyCardGroup extends CardGroup {
    ...
    public void refreshHandLayout() {
        ...
        for (int i = 0; i < this.group.size(); i++) {
            ...
            c.target_x = Settings.WIDTH * .9F - ((cardsinrow + 0.5f) * (widthspacing * AbstractRuinaBossCard.HAND_SCALE)) + (widthspacing * AbstractRuinaBossCard.HAND_SCALE) * (i % HAND_ROW_LENGTH);
            c.target_y = Settings.HEIGHT * HAND_HEIGHT_OFFSET + (AbstractCard.IMG_HEIGHT_S * AbstractRuinaBossCard.HAND_SCALE) * ((float) Math.floor(((float) i) / (float) HAND_ROW_LENGTH) + (this.group.size() > HAND_ROW_LENGTH ? 0.0f : 1.0f));
    ...
    public void hoverCardPush(final AbstractCard c) {
        ...
        if (cardrow.size() > 1) {
            ...
            pushAmt *= AbstractRuinaBossCard.HAND_SCALE / 0.75f;
