...
public abstract class AbstractRuinaBossCard extends AbstractCard {
    ...
    public AbstractRuinaCardMonster owner;
    ...
    public AbstractRuinaBossCard(AbstractCard baseCard) {
        super(makeID(baseCard.cardID), baseCard.name, baseCard.assetUrl, baseCard.cost, baseCard.rawDescription, baseCard.type,
    ...
    public static int statusValue() {
        int value = -5;
        if (AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT && AbstractRuinaCardMonster.boss != null) {
            if (AbstractRuinaCardMonster.boss.hasPower(EvolvePower.POWER_ID)) {
                value = 0;
            } else if (AbstractRuinaCardMonster.boss.hasRelic("Medical Kit")) {
                value += 4;
            }
        }
        return value;
    ...
    public int autoPriority() {
        AbstractRuinaCardMonster ownerBoss = owner;
        boolean setupPhase = ownerBoss.onSetupTurn;
        float blockModifier = 1F;
        if (setupPhase) blockModifier = 1.5F;
        int value = 0;
        if (this.type == CardType.STATUS) { value += -10;
        } else if (this.type == CardType.CURSE && this.costForTurn < -1) { value += -100; }
        if (this.type == CardType.ATTACK) {
            if (ownerBoss.currentHealth > ownerBoss.maxHealth / 2) { value += Math.max(this.damage, 0);
            } else { value += Math.max(this.damage * 2.0f, 0); }
        }
        value += Math.max(this.block * 1.3F * blockModifier, 0);
        if (this.type == CardType.POWER || this.color == CardColor.COLORLESS) {
            if (ownerBoss.currentHealth > ownerBoss.maxHealth / 2) { value *= 5;
            } else { value /= 2; }
        }
    ...
    public void applyPowers() {
        ...
        for (final AbstractPower p : player.powers) { tmp = p.atDamageReceive(tmp, this.damageTypeForTurn, this); }
        for (final AbstractPower p : this.owner.powers) { tmp = p.atDamageFinalGive(tmp, this.damageTypeForTurn, this); }
        for (final AbstractPower p : player.powers) { tmp = p.atDamageFinalReceive(tmp, this.damageTypeForTurn, this); }
        ...
        if (this.intent != null) {
            if (!this.bossDarkened) { createIntent(); }
        }
        if (AbstractRuinaCardMonster.boss != null) {
            if (AbstractRuinaCardMonster.boss.hasPower(StunMonsterPower.POWER_ID)) {
                bossDarken();
                destroyIntent();
            }
        }
    ...
    public void atb(AbstractGameAction action) { addToBot(action); }
    ...
    public void calculateCardDamage(AbstractMonster mo) {
        ...
        if (mo == null) {
            mo = this.owner;
        } else if (this.owner == null && mo instanceof AbstractRuinaCardMonster) {
            this.owner = (AbstractRuinaCardMonster) mo;
        }
        if (mo != null) {
            float tmp = (float) this.baseDamage;
            for (final AbstractRelic r : this.owner.relics) {
            ...
            for (final AbstractPower p : this.owner.powers) { tmp = p.atDamageGive(tmp, this.damageTypeForTurn, this); }
            for (final AbstractPower p : player.powers) { tmp = p.atDamageReceive(tmp, this.damageTypeForTurn, this); }
    ...
    public void triggerOnEndOfPlayerTurn() {
        if (this.isEthereal) { this.addToTop(new ExhaustSpecificCardAction(this, AbstractRuinaCardMonster.boss.hand)); }
    }
    ...
    public boolean hasEnoughEnergy() {
        ...
        if (AbstractRuinaCardMonster.boss.hasPower("Entangled") && this.type == CardType.ATTACK) {
    ...
    public boolean canUse(final AbstractPlayer p, final AbstractMonster m) {
        if (m instanceof AbstractRuinaCardMonster) {
            AbstractRuinaCardMonster cB = (AbstractRuinaCardMonster) m;
            if ((this.type == CardType.STATUS) && (this.costForTurn < -1) && (cB.hasRelic("Medical Kit"))) { return true; }
            if ((this.type == CardType.CURSE) && (this.costForTurn < -1) && (cB.hasRelic("Blue Candle"))) { return true; }
    ...
    public void hover() {
        ...
        if (!this.hov2) {
            ...
            this.targetDrawScale = ruina.monsters.eventBoss.AbstractRuinaBossCard.HOVER_SCALE;
            this.drawScale = ruina.monsters.eventBoss.AbstractRuinaBossCard.HOVER_SCALE;
        }
    }
    ...
    public void unhover() {
        ...
        if (this.hov2) {
            ...
            AbstractRuinaCardMonster.boss.hand.refreshHandLayout();
            this.targetDrawScale = ruina.monsters.eventBoss.AbstractRuinaBossCard.HAND_SCALE;
        }
    }
    ...
    public void update() {
        ...
        if (this.bossDarkened) { ReflectionHacks.setPrivate(this, AbstractCard.class, "tintColor", new Color(255F * .43F, 255F * .37F, 255F * .65F, 0F)); }
    }
    ...
    private void updateIntentTip() {
        switch (this.intent) {
            case ATTACK:
                ...
                if (this.isMultiDamage) { this.intentTip.body = TEXT[1] + this.intentDmg + TEXT[2] + this.intentMultiAmt + TEXT[3];
                } else { this.intentTip.body = TEXT[4] + this.intentDmg + TEXT[5]; }
    ...
    public String overrideIntentText() {
        if (this.isMultiDamage) { return intentDmg + "x" + intentMultiAmt;
        } else { return Integer.toString(intentDmg); }
    }
    ...
    static { TEXT = CardCrawlGame.languagePack.getUIString("AbstractMonster").TEXT; }
}