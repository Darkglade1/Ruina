...
public abstract class AbstractBossDeckArchetype {
    ...
    public ArrayList<AbstractRuinaBossCard> cards;
    protected AbstractCharbossRelic starterRelic;
    private ArrayList<AbstractRuinaBossCard> allCards;
    private ArrayList<AbstractRuinaBossCard> curseCards;
    private ArrayList<AbstractRuinaBossCard> synergyCards;
    public AbstractRuinaBossCard[] signatureCardPerAct;
    private ArrayList<AbstractRuinaBossCard> classGlobalCards;
    private ArrayList<AbstractRuinaBossCard> starterCards;
    ...
    */
    ...
    public AbstractBossDeckArchetype(String id, String loggerClassName, String loggerArchetypeName) {
        ...
        /*
        this.ID = id;
        this.allCards = new ArrayList<AbstractRuinaBossCard>();
        this.starterCards = new ArrayList<AbstractRuinaBossCard>();
        this.classGlobalCards = new ArrayList<AbstractRuinaBossCard>();
        this.synergyCards = new ArrayList<AbstractRuinaBossCard>();
        this.signatureCardPerAct = new AbstractRuinaBossCard[3];
        ...
        this.curseCards = new ArrayList<AbstractRuinaBossCard>();
        ...
        this.cards = new ArrayList<AbstractRuinaBossCard>();
        */
    ...
    public void addToDeck(AbstractRuinaBossCard c) {
        addToDeck(c, false);
    }
    public void addToDeck(AbstractRuinaBossCard c, boolean upgraded) {
        if (upgraded) c.upgrade();
        AbstractRuinaCardMonster.boss.masterDeck.addToTop(c.makeStatEquivalentCopy());
    }
*/
    ...
    private static AbstractRuinaBossCard getRandomCardFromSource(ArrayList<AbstractRuinaBossCard> source) {
        return (AbstractRuinaBossCard) source.get(AbstractDungeon.monsterRng.random(source.size() - 1)).makeCopy();
    }
    ...
    */
    ...
    public String addRandomCurse(AbstractRuinaCardMonster boss, String loggerSource) {
    ...
    protected void addCardToList(AbstractRuinaBossCard c, CardBenefitType type) {
        switch (type) {
            case SIGNATUREACT1:
    ...
    protected void addToStarterDeck(AbstractRuinaBossCard c) {
        this.starterCards.add(c);
    }
    private ArrayList<AbstractRuinaBossCard> sortCardListToRarity(ArrayList<AbstractRuinaBossCard> cards) {
        ArrayList<AbstractRuinaBossCard> sortedCards = new ArrayList<AbstractRuinaBossCard>();
        boolean[] validRarities = new boolean[3];
        outer:
        for (AbstractRuinaBossCard c : cards) {
    ...
    private AbstractRuinaBossCard getStrikeOrDefend() {
        for (AbstractRuinaBossCard c : this.starterCards) {
    ...
    private AbstractRuinaBossCard getCardToUpgrade(ArrayList<AbstractRuinaBossCard> cards) {
    ...
    public void simulateBuild(AbstractRuinaCardMonster boss) {
        ...
        if (this.blacklistedCards.size() > 0) {
            for (String s : this.blacklistedCards) {
                ArrayList<AbstractRuinaBossCard> cardsToRemove = new ArrayList<>();
                for (AbstractRuinaBossCard c : classGlobalCards) {
                    if (c.cardID.equals(s)) {
                        cardsToRemove.add(c);
                    }
                }
                if (cardsToRemove.size() > 0) {
                    for (AbstractRuinaBossCard c : cardsToRemove) {
                        if (c.cardID.equals(s)) {
        ...
        for (AbstractRuinaBossCard c : this.synergyCards) {
            logger.info(c.name);
        }
        logger.info("Boss's potential Class Global Cards:");
        for (AbstractRuinaBossCard c : this.classGlobalCards) {
            logger.info(c.name);
        }
        ...
        for (int actIndex = 0; actIndex < AbstractDungeon.actNum; actIndex++) {
            ...
            AbstractBossDeckArchetype.logger.info("Boss's current max HP is " + AbstractRuinaCardMonster.boss.maxHealth);
            ...
            if (cardRemovalsPerAct[actIndex] > 0) {
                for (int i2 = 0; i2 < cardRemovalsPerAct[actIndex]; i2++) {
                    AbstractRuinaBossCard cut = getStrikeOrDefend();
    ...
    public void eggCheck(AbstractRuinaBossCard c) {
    ...
    public String upgradeRandomCard(String debugName) {
        AbstractRuinaBossCard up = getCardToUpgrade(cards);
        if (up != null) {
    ...
    public String addRandomGlobalRelic(int actIndex, AbstractRuinaCardMonster boss, String loggerName, ArrayList<AbstractRuinaBossCard> cards) {
    ...
    public String addRandomGlobalRelicOfRarity(AbstractRuinaCardMonster boss, String loggerName, AbstractRelic.RelicTier tier) {
        logger.info("Global relic has been requested by " + loggerName + ", global pool size = " + this.globalRelicPool.size());
        if (this.globalRelicPool.size() > 0) {
            ArrayList<AbstractCharbossRelic> sortedRelics = sortRelicListToRarity(this.globalRelicPool);
    ...
    public String addRandomGlobalRelic(int actIndex, AbstractRuinaCardMonster boss, ArrayList<AbstractRuinaBossCard> cards) {
        String name = addRandomGlobalRelic(actIndex, boss, "", cards);
        return name;
    }
    public void addRandomGlobalRelic(AbstractRuinaCardMonster boss, ArrayList<AbstractRuinaBossCard> cards) {
        addRandomGlobalRelic(0, boss, "", cards);
    }
    public void addRandomGlobalRelic(AbstractRuinaCardMonster boss, ArrayList<AbstractRuinaBossCard> cards, AbstractRelic.RelicTier tier) {
        addRandomGlobalRelic(0, boss, "", cards);
    }
    public void addSpecificRelic(AbstractCharbossRelic relic, AbstractRuinaCardMonster boss, String loggerName, ArrayList<AbstractRuinaBossCard> cards) {
    ...
    public String addRandomGlobalClassCard(String debugName) {
        if (this.classGlobalCards.size() > 0) {
            ArrayList<AbstractRuinaBossCard> sortedCards = sortCardListToRarity(this.classGlobalCards);
            int random = AbstractDungeon.cardRng.random(0, sortedCards.size() - 1);
            AbstractRuinaBossCard randomCard = sortedCards.get(random);
            eggCheck(randomCard);
            cards.add((AbstractRuinaBossCard) randomCard.makeCopy());
    ...
    public void addSpecificCard(String debugName, AbstractRuinaBossCard card) {
        eggCheck(card);
        cards.add((AbstractRuinaBossCard) card.makeCopy());
        logger.info(debugName + " added " + card.name + ".");
    }
    public String addRandomSynergyCard(String debugName) {
        if (this.synergyCards.size() > 0) {
            ArrayList<AbstractRuinaBossCard> sortedCards = sortCardListToRarity(this.synergyCards);
            int random = AbstractDungeon.cardRng.random(0, sortedCards.size() - 1);
            AbstractRuinaBossCard randomCard = sortedCards.get(random);
            eggCheck(randomCard);
            cards.add((AbstractRuinaBossCard) randomCard.makeCopy());
    ...
    public AbstractRuinaBossCard getRandomCard() {
        return getRandomCardFromSource(this.allCards);
    }
    public AbstractRuinaBossCard getRandomCard(ArrayList<AbstractRuinaBossCard> cards) {
        return this.getRandomCard(cards, this.allCards);
    }
    public AbstractRuinaBossCard getRandomCard(ArrayList<AbstractRuinaBossCard> cards, ArrayList<AbstractRuinaBossCard> source) {
        AbstractRuinaBossCard c = getRandomCardFromSource(source);
        if (c.limit <= cards.size()) {
            int count = 0;
            for (AbstractRuinaBossCard actIndex : cards) {
    ...
    public AbstractRuinaBossCard getRandomCard(ArrayList<AbstractRuinaBossCard> cards, ArrayList<AbstractRuinaBossCard> source, CardRarity rarity) {
        return this.getRandomCard(cards, source, rarity, false);
    }
    public AbstractRuinaBossCard getRandomCard(ArrayList<AbstractRuinaBossCard> cards, CardRarity rarity) {
        return this.getRandomCard(cards, this.allCards, rarity, false);
    }
    public AbstractRuinaBossCard getRandomCard(ArrayList<AbstractRuinaBossCard> cards, CardRarity rarity, boolean blacklist) {
        return this.getRandomCard(cards, this.allCards, rarity, blacklist);
    }
    public AbstractRuinaBossCard getRandomCard(ArrayList<AbstractRuinaBossCard> cards, ArrayList<AbstractRuinaBossCard> source, CardRarity rarity, boolean blacklist) {
        AbstractRuinaBossCard c = getRandomCardFromSource(source);
