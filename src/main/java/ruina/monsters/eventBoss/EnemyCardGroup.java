...
public class EnemyCardGroup extends CardGroup {
    ...
    public static AbstractRuinaBossCard hov2holder = null;
    ...
    public EnemyCardGroup(CardGroupType type, AbstractRuinaCardMonster owner) {
        super(type);
        this.owner = owner;
    }
    ...
    public EnemyCardGroup(CardGroup group, CardGroupType type, AbstractRuinaCardMonster owner) {
        super(group, type);
        this.owner = owner;
    }
    ...
    public void moveToHand(final AbstractCard c, final CardGroup group) {
        ...
        c.setAngle(0.0f);
        c.drawScale = 0.12f;
        c.targetDrawScale = ruina.monsters.eventBoss.AbstractRuinaBossCard.HAND_SCALE;
    ...
    public void initializeDeck(final CardGroup masterDeck) {
        ...
        if (placeOnTop.size() > AbstractDungeon.player.masterHandSize) {
            //AbstractDungeon.actionManager.addToTurnStart(new EnemyDrawCardAction(AbstractRuinaCardMonster.boss, placeOnTop.size() - AbstractRuinaCardMonster.boss.masterHandSize));
        }
        placeOnTop.clear();
    }
    ...
    public void refreshHandLayout() {
        ...
        for (int i = 0; i < this.group.size(); i++) {
            ...
            c.target_x = Settings.WIDTH * .9F - ((cardsinrow + 0.5f) * (widthspacing * ruina.monsters.eventBoss.AbstractRuinaBossCard.HAND_SCALE)) + (widthspacing * ruina.monsters.eventBoss.AbstractRuinaBossCard.HAND_SCALE) * (i % HAND_ROW_LENGTH);
            c.target_y = Settings.HEIGHT * HAND_HEIGHT_OFFSET + (AbstractCard.IMG_HEIGHT_S * ruina.monsters.eventBoss.AbstractRuinaBossCard.HAND_SCALE) * ((float) Math.floor(((float) i) / (float) HAND_ROW_LENGTH) + (this.group.size() > HAND_ROW_LENGTH ? 0.0f : 1.0f));
            if (((AbstractRuinaBossCard) c).hov2 && c.hb.hovered) {
    ...
    public void glowCheck() {
        for (final AbstractCard c : this.group) {
            if (c.canUse(AbstractDungeon.player, AbstractRuinaCardMonster.boss) && AbstractDungeon.screen != AbstractDungeon.CurrentScreen.HAND_SELECT) {
                c.beginGlowing();
            } else {
                c.stopGlowing();
            }
    ...
    public void hoverCardPush(final AbstractCard c) {
        ...
        if (cardrow.size() > 1) {
            ...
            pushAmt *= ruina.monsters.eventBoss.AbstractRuinaBossCard.HAND_SCALE / 0.75f;
    ...
    public AbstractRuinaBossCard getHighestValueCard() {
        AbstractRuinaBossCard r = null;
        int record = -99;
        for (AbstractCard c : this.group) {
            AbstractRuinaBossCard cc = (AbstractRuinaBossCard) c;
    ...
    public AbstractRuinaBossCard getHighestValueCard(AbstractCard.CardType type) {
        AbstractRuinaBossCard r = null;
        int record = -99;
        for (AbstractCard c : this.group) {
            if (c.type == type) {
    ...
    public AbstractRuinaBossCard getHighestUpgradeValueCard() {
        AbstractRuinaBossCard r = null;
        int record = -99;
        for (AbstractCard c : this.group) {
            AbstractRuinaBossCard cc = (AbstractRuinaBossCard) c;
